...
async function callJSON(system, user, useWebSearch = false) {
  ...
  let clean = t.replace(/```json\s*/g, "").replace(/```/g, "").trim();
  // If the model added narrative before or after the JSON, extract the first
  // complete JSON array or object
  const arrMatch = clean.match(/\[[\s\S]*\]/);
  const objMatch = clean.match(/\{[\s\S]*\}/);
  const jsonStr = arrMatch ? arrMatch[0] : objMatch ? objMatch[0] : clean;
  try {
